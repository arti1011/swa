	...
	public List<Bestellung> findBestellungenByKunde(AbstractKunde kunde, FetchType fetch) {
		...
		case NUR_BESTELLUNG:
			bestellungen = em.createNamedQuery(Bestellung.FIND_BESTELLUNGEN_BY_KUNDEID, Bestellung.class)
					.setParameter(Bestellung.PARAM_KUNDEID, kunde.getId()).getResultList();
			break;
		case MIT_LIEFERUNGEN:
			bestellungen = em
					.createNamedQuery(Bestellung.FIND_BESTELLUNGEN_BY_KUNDEID_FETCH_LIEFERUNGEN, Bestellung.class)
					.setParameter(Bestellung.PARAM_KUNDEID, kunde.getId()).getResultList();
			break;
		default:
