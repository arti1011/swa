...
public class BestellungResource {
	...
	private Link[] getTransitionalLinks(Bestellung bestellung, UriInfo uriInfo) {
		final Link self = Link.fromUri(getUriBestellung(bestellung, uriInfo)).rel(SELF_LINK).build();
		final Link add = Link.fromUri(uriHelper.getUri(BestellungResource.class, uriInfo)).rel(ADD_LINK).build();
	...
	/**
	 * Mit der URL /bestellungen/{id}/lieferungen die Lieferung ermitteln zu einer bestimmten Bestellung ermitteln
	 * 
	 * @param id
	 *            ID der Bestellung
	...
	public Response findLieferungenByBestellungId(@PathParam("id") Long id) {
		...
		return Response.status(INTERNAL_SERVER_ERROR).entity("findLieferungenByBestellungId: NOT YET IMPLEMENTED")
