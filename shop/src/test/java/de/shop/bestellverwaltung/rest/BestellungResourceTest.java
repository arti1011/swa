...
public class BestellungResourceTest extends AbstractResourceTest {
	...
	public void findBestellungById() {
		...
		// When
		final Response response = getHttpsClient(USERNAME_ADMIN, PASSWORD_ADMIN).target(BESTELLUNGEN_ID_URI)
				.resolveTemplate(BESTELLUNGEN_ID_PATH_PARAM, bestellungId).request().accept(APPLICATION_JSON)
				.get();
	...
	public void createBestellung() throws URISyntaxException {
		...
		// When
		Long id;
		Response response = getHttpsClient(USERNAME_ADMIN, PASSWORD_ADMIN).target(BESTELLUNGEN_URI).request()
		...
		response = getHttpsClient(USERNAME_ADMIN, PASSWORD_ADMIN).target(BESTELLUNGEN_ID_URI)
				.resolveTemplate(BESTELLUNGEN_ID_PATH_PARAM, id).request().accept(APPLICATION_JSON).get();
		assertThat(response.getStatus()).isEqualTo(HTTP_OK);
		response.close();
